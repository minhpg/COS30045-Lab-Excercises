...
const drawGraph = () => {
  ...
  xScale = d3
    ...
    .range([0, innerWidth]);
  /** Draw bars */
  let bars = svg.selectAll("rect").data(currentDataset);

  if (sortEnabled)
    bars = bars.sort((a, b) =>
      sortAscending ? d3.ascending(a, b) : d3.descending(a, b)
    );
...
/** Sort order checkbox event listener */
d3.select("#sort-order").on("change", (event) => {
  const checked = d3.select("#sort-order").property("checked");
  sortAscending = !checked;
  drawGraph();
});

/** Sort enabled checkbox event listener */
d3.select("#sort-checkbox").on("change", (event) => {
  const checked = d3.select("#sort-checkbox").property("checked");
  sortEnabled = checked;
  if (!sortEnabled)
    currentDataset = currentDataset.sort(() => Math.random() - 0.5);
  drawGraph();
});
